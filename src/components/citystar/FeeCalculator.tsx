...
export function FeeCalculator({ onContact }: { onContact: (selection: Selection) => void }) {
  ...
  const selection = (): Selection => ({
    ...
    lignes: [
      ...
      connus && frais !== null ? t.frais.fraisEstimes(formatPrix(Math.round(frais), devise, langue)) : t.frais.tauxAttente,
    ],
  });
    ...
    <section id="frais" className="cl section-pad" aria-labelledby="frais-title">
      <div className="cl-grid">
        <div className="cl-form">
          ...
          <h2 id="frais-title">{t.frais.titre[0]}<br /><em>{t.frais.titre[1]}</em></h2>
          ...
          <div className="cl-field">
            ...
            <input className="cl-range" type="range" min={min} max={max} step={pas} value={montant} onChange={(event) => changePrix(Number(event.target.value))} aria-label={t.frais.prixCurseur} />
          </div>
          ...
          <div className="cl-field">
            ...
            <select id="cl-villa" value={villa} onChange={(event) => changeVilla(event.target.value as TypeVilla | "")}>
              ...
              {TYPES.map((type) => <option key={type} value={type}>{t.villas.villa} {type}</option>)}
            </select>
          </div>
        ...
        <div className="cl-side">
          <article className={`cl-doc${connus ? "" : " is-neutral"}`}>
            <header>
              <div><b>CITYSTAR</b><small>{t.frais.residence}</small></div>
              <p>{date}<br />{villa ? `${t.villas.villa} ${villa}` : t.frais.villaNonPrecisee}</p>
            </header>
            <h3>{t.frais.titreFiche}</h3>
            <table>
              <tbody>
                <tr><td>{t.frais.prix}</td><td>{formatPrix(montant, devise, langue)}</td></tr>
                {taux.map((ligne) => (
                  <tr key={ligne.key}>
                    ...
                    <td>{ligne.valeur === null ? <span className="cl-wait">{t.frais.attente}</span> : formatPrix(Math.round(montant * ligne.valeur), devise, langue)}</td>
                  </tr>
                ))}
                <tr className="cl-total"><td>{t.frais.total}</td><td>{frais === null ? "—" : formatPrix(Math.round(montant + frais), devise, langue)}</td></tr>
              </tbody>
            </table>
            <p className="cl-mention"><b>{t.frais.mention[0]}</b>{t.frais.mention[1]}</p>
            <span className="cl-stamp" aria-hidden="true">{t.frais.cachet[0]}<br />{t.frais.cachet[1]}</span>
          </article>

          {connus ? (
            <div className="cl-actions">
              <PillButton label={t.frais.email} icon={ArrowRight} onClick={() => onContact(selection())} />
              <PillButton label={t.frais.imprimer} icon={Printer} variant="secondary" onClick={() => window.print()} />
            </div>
          ) : (
            <div className="cl-actions">
              <p className="cl-neutral">{t.frais.neutre}</p>
              <PillButton label={t.frais.personnalisee} icon={ArrowRight} onClick={() => onContact(selection())} />
            </div>
          )}
        </div>
      </div>
