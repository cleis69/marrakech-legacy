...
export function LocationSection({ onOpenPlan }: { onOpenPlan: (src: string) => void }) {
  ...
  const places: {
  ...
  }[] = [
    {
      ...
      title: lieux.med.titre,
      note: lieux.med.note(programme.trajetMaxMinutes),
    },
    {
      id: "air",
      minutes: programme.trajetMaxMinutes,
      title: lieux.air.titre,
      note: lieux.air.note(programme.trajetMaxMinutes),
    },
    {
      id: "palm",
      minutes: null,
      word: lieux.palm.mot,
      title: lieux.palm.titre,
      note: lieux.palm.note,
    },
  ];
      ...
      <div className="loc-grid">
        ...
        <div className="loc-aside">
          <ul className="loc-dist">
            {places.map((place) => (
              <li key={place.id}>
                ...
                >
                  <span className={`loc-n${place.minutes === null ? " is-word" : ""}`}>
                    {place.minutes ?? place.word}
                    {place.minutes !== null && <small>min</small>}
                  </span>
                  <span className="loc-l">
                    <b>{place.title}</b>
          ...
          <div className="loc-foot">
            <PillButton
              label={t.localisation.planMasse}
              icon={Expand}
              variant="secondary"
              onClick={() => onOpenPlan(masterplanImage)}
            />
