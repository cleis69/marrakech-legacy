      ...
      <div className="pj-grid">
        <figure className="pj-big">
          ...
          <figcaption className="pj-count">
            <VillaCounter />
            <span className="pj-count-copy"><small>{t.projet.villas(programme.nombreVillas)}</small><em>{t.projet.pasUne}</em></span>
          </figcaption>
        </figure>
        ...
        <div className="pj-right">
          ...
          <div className="pj-text">
            <div className="section-label"><span>01</span><p>{t.projet.label}</p></div>
            <Reveal><h2 id="project-title">{t.projet.titre[0]}<br />{t.projet.titre[1]}<em>{t.projet.titre[2]}</em></h2></Reveal>
            <p>{t.projet.texte}</p>
            <dl className="pj-rows">
              <div><dt>{t.projet.terrains}</dt><dd>{formatNombre(programme.terrainMaxM2, langue)}<sup>m²</sup></dd></div>
              <div><dt>{t.projet.types}</dt><dd>{programme.nombreTypes}</dd></div>
              <div><dt>{t.projet.trajet}</dt><dd>{programme.trajetMaxMinutes}<sup>min</sup></dd></div>
            </dl>
            <PillButton label={t.projet.cta} icon={ArrowRight} variant="secondary" onClick={() => scrollTo("villas")} />
          </div>
        </div>
      </div>
    </section>
