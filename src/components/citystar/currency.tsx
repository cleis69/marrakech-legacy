...
type DeviseState = {
  devise: Devise;
  setDevise: (devise: Devise) => void;
  langue: Langue;
  t: Textes;
  taux: Taux;
};

const DeviseContext = createContext<DeviseState>({
  devise: "EUR",
  setDevise: () => {},
  langue: "fr",
  t: textes.fr,
  taux: devises.tauxDeSecours,
});
...
export function DeviseProvider({
  ...
  useEffect(() => {
    ...
    let vivant = true;
    void recupererTaux().then((frais) => {
      if (vivant && frais) setTaux(frais);
    });
    return () => {
      vivant = false;
    };
  ...
  const value = useMemo(
    () => ({ devise, setDevise, langue, t: textes[langue], taux }),
    [devise, langue, taux],
  );
    ...
    <div className={`cur-pills ${className}`} role="group" aria-label={t.prix.devise}>
      {devises.affichees.map((code) => (
        <button
          key={code}
          type="button"
          aria-pressed={code === devise}
          onClick={() => setDevise(code)}
          aria-label={t.prix.devises[code]}
        >
          {symbolesDevise[code]}
        </button>
      ))}
...
export function VillaPrice({ type }: { type: TypeVilla }) {
  ...
  return (
    <div className="vprice">
      ...
      <strong className="vprice-main" aria-live="polite">
        {approximatif ? "≈ " : ""}
        {formatPrix(montant, devise, langue)}
      </strong>
      <CurrencyPills />
      {devise !== "EUR" && (
        <span className="vprice-ref">{t.prix.reference(formatPrix(reference, "EUR", langue))}</span>
      )}
      {approximatif && (
