...
function justify(type: TypeVilla, a: Answers, textes: Textes, langue: Langue) {
  ...
  if (villasChiffres[type].accessiblePmr && a.pmr === "yes") {
    phrase = j.pmr(taille);
    if (souhaite !== null && souhaite !== villasChiffres[type].suites)
...
export function VillaSelector({ onContact }: { onContact: (selection: Selection) => void }) {
  ...
  const questions: Question[] = [
    {
      ...
      options: [
        {
          value: "vivre",
          label: t.selecteur.questions.usage.vivre,
          hint: t.selecteur.questions.usage.vivreNote,
        },
        {
          value: "investir",
          label: t.selecteur.questions.usage.investir,
          hint: t.selecteur.questions.usage.investirNote,
        },
      ],
    },
    {
      key: "suites",
      title: t.selecteur.questions.suites.titre,
      options: [
        ...suites.map((n) => ({ value: String(n), label: t.selecteur.questions.suites.suites(n) })),
        { value: "any", label: t.selecteur.questions.suites.peuImporte },
      ],
    },
    {
      key: "budget",
      ...
      options: [
        ...
        })),
        {
          ...
          hint: t.selecteur.questions.budget.parlerNote,
        },
      ],
    ...
    },
    {
      ...
      options: [
        {
          value: "yes",
          label: t.selecteur.questions.pmr.oui,
          hint: t.selecteur.questions.pmr.ouiNote,
        },
        { value: "no", label: t.selecteur.questions.pmr.non },
      ],
    },
  ];
  ...
  const back = () => {
    ...
    setAnswers((current) => {
      const next = { ...current };
      if (previous) delete next[previous.key];
      return next;
    });
    setDir(-1);
    setStep((s) => s - 1);
  ...
  const selection = (): Selection => ({
    ...
    lignes: [
      ...questions.map((q) => labelOf(q.key)).filter(Boolean),
      t.selecteur.ligneRecommandation(winner ?? "B"),
    ],
  });
  ...
  return (
    <section
      id="selecteur"
      className="sl section-pad"
      aria-labelledby="selecteur-title"
      onKeyDown={onKeyDown}
    >
        ...
        <div className="sl-pane">
          <div className="sl-head">
            <div className="sl-head-row">
              <h2 id="selecteur-title" className="sl-kicker">
                {t.selecteur.kicker}
              </h2>
              <button type="button" className="sl-back" onClick={back} disabled={step === 0}>
                <ArrowLeft aria-hidden="true" /> {t.selecteur.retour}
              </button>
            </div>
            <div className="sl-progress">
              <span>
                {done ? t.selecteur.recommandation : t.selecteur.question(step + 1, STEPS)}
              </span>
              <span
                className="sl-line"
                role="progressbar"
                aria-label={t.selecteur.progression}
                aria-valuemin={0}
                aria-valuemax={STEPS}
            ...
            >
              {!done && question ? (
                <Stage focusPending={focusPending}>
                  ...
                  <p className="sl-keys" aria-hidden="true">
                    {t.selecteur.clavier}
                  </p>
                </Stage>
              ) : winner ? (
        ...
        <ul className="sl-trio" aria-label={t.selecteur.apercu}>
          {villas.map((villa) => {
            ...
            return (
              <li
                key={type}
                className={`sl-card${reason ? " is-dim" : ""}${type === winner ? " is-win" : ""}`}
              >
...
function Stage({
  ...
  focusPending: React.MutableRefObject<boolean>;
  ...
  return (
    <div ref={ref} className="sl-stage">
      {children}
    </div>
  );
}
