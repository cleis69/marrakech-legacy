...
const ITINERARY_URL = "https://www.google.com/maps/dir/?api=1&destination=Oulad+Hassoune,+Marrakech";
...
const GEOMETRY = (() => {
  ...
  for (let c = -3; c < 13; c++) {
    const y0 = 60 + c * 75;
    let d = `M-300,${y0}`;
    for (let x = -300; x <= 1300; x += 80) d += ` L${x},${(y0 + Math.sin(x / 140 + c) * 22 + (rnd() - 0.5) * 14).toFixed(1)}`;
    contours.push(d);
  }
  const palms = Array.from({ length: 110 }, () => {
    const a = rnd() * Math.PI * 2, r = Math.sqrt(rnd());
    return { cx: (600 + Math.cos(a) * r * 175).toFixed(1), cy: (320 + Math.sin(a) * r * 78).toFixed(1), r: (1.6 + rnd() * 2.2).toFixed(1) };
  });
  const details: string[] = [];
  for (let j = 0; j < 16; j++) {
    const sx = 375 + rnd() * 100, sy = 385 + rnd() * 105;
    details.push(`M${sx.toFixed(0)},${sy.toFixed(0)} l${((rnd() - 0.5) * 40).toFixed(0)},${((rnd() - 0.5) * 40).toFixed(0)} l${((rnd() - 0.5) * 30).toFixed(0)},${((rnd() - 0.5) * 30).toFixed(0)}`);
  }
  for (let t = 0; t < 5; t++) {
    const ty = 270 + t * 22;
...
export function LocationMap({ selected, onSelect }: Props) {
  ...
  const st = useRef({ cur: { ...HOME }, tgt: { ...HOME }, raf: 0, route: 0, hint: 0, drag: null as Drag | null, mounted: false });
  ...
  const apply = () => {
    ...
    if (!box || !svg) return;
    const w = box.clientWidth || 1, h = box.clientHeight || 1, k = scale(w);
    const { cur } = st.current;
    const vw = 1000 / cur.z, vh = (vw * h) / w, vx = cur.x - vw / 2, vy = cur.y - vh / 2;
    ...
    svg.querySelectorAll<SVGGElement>("[data-x]").forEach((mark) => {
      mark.setAttribute("transform", `translate(${mark.dataset["x"]} ${mark.dataset["y"]}) scale(${(k / cur.z).toFixed(3)})`);
    });
    const pop = popRef.current;
    if (pop) {
      // La fiche passe sous l'étoile quand elle manquerait de place au-dessus.
      const starY = ((STAR.y - vy) / vh) * h, radius = (30 * k * w) / 1000;
  ...
  const tick = () => {
    ...
    (["x", "y", "z"] as const).forEach((p) => {
      ...
      if (Math.abs(d) > (p === "z" ? 0.002 : 0.3)) { s.cur[p] += d * (reduce ? 1 : 0.14); done = false; } else s.cur[p] = s.tgt[p];
    });
    apply();
  ...
  const route = (place: Place) => {
    const svg = svgRef.current, box = boxRef.current, live = liveRef.current, head = headRef.current;
    const geo = svg?.querySelector<SVGPathElement>(`[data-route="${place}"]`);
    if (!svg || !box || !live || !head || !geo) return;
    clearRoute();
    setPopOpen(false);
    const len = geo.getTotalLength(), bb = geo.getBBox();
    const w = box.clientWidth || 1, h = box.clientHeight || 1, k = scale(w);
    st.current.tgt = { x: bb.x + bb.width / 2, y: bb.y + bb.height / 2, z: Math.min((1000 * 0.5) / bb.width, (1000 * (h / w) * 0.5) / bb.height) };
    go();
    const minutes = programme.trajetMaxMinutes;
    const t0 = performance.now() + (reduce ? 0 : 450), duration = reduce ? 1 : 2200;
    head.style.display = "";
    const step = () => {
  ...
  useEffect(() => {
    ...
    const onWheel = (event: WheelEvent) => {
      if (!(event.ctrlKey || event.metaKey)) {
        ...
        st.current.hint = window.setTimeout(() => hintRef.current?.classList.remove("is-flash"), 900);
        return;
      }
      event.preventDefault();
      const { cur, tgt } = st.current;
      const rect = box.getBoundingClientRect(), w = rect.width || 1, h = rect.height || 1;
      const mx = (event.clientX - rect.left) / w, my = (event.clientY - rect.top) / h;
      const vw = 1000 / cur.z, vh = (vw * h) / w, wx = cur.x - vw / 2 + mx * vw, wy = cur.y - vh / 2 + my * vh;
      const nz = clamp(cur.z * (event.deltaY < 0 ? 1.12 : 1 / 1.12), LIMITS.z[0], LIMITS.z[1]);
      const nvw = 1000 / nz, nvh = (nvw * h) / w;
      cur.z = tgt.z = nz;
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const { cur } = st.current;
    st.current.drag = { px: event.clientX, py: event.clientY, x: cur.x, y: cur.y, moved: false, target };
    event.currentTarget.setPointerCapture(event.pointerId);
    event.currentTarget.classList.add("is-dragging");
  };

  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    const drag = st.current.drag, box = boxRef.current;
    if (!drag || !box) return;
  ...
  return (
    <div ref={boxRef} className={`mp${popOpen ? " has-pop" : ""}`} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerEnd} onPointerCancel={onPointerEnd}>
      <svg ref={svgRef} className="mp-svg" viewBox="0 0 1000 700" role="img" aria-label={t.localisation.carteAria}>
        ...
        {GEOMETRY.palms.map((p, i) => <circle key={i} className="mp-palm" cx={p.cx} cy={p.cy} r={p.r} />)}
        <g className="mp-detail">{GEOMETRY.details.map((d) => <path key={d} d={d} />)}</g>
        <polygon className="mp-medina" points="385,380 455,372 492,410 488,478 440,505 380,492 360,440" />
        <rect className="mp-runway" x="235" y="582" width="110" height="8" transform="rotate(-24 290 586)" />
        {(Object.keys(ROUTES) as Place[]).map((place) => <path key={place} className="mp-geo" data-route={place} d={ROUTES[place]} />)}
        ...
        <g className="mp-star" data-x={STAR.x} data-y={STAR.y}>
          ...
          <path className="mp-star-shape" d="M0,-28C2,-8 8,-2 28,0 8,2 2,8 0,28 -2,8 -8,2 -28,0 -8,-2 -2,-8 0,-28Z" />
          <text className="mp-big" y="-46" textAnchor="middle">CITYSTAR</text>
          <text y="58" textAnchor="middle">Oulad Hassoune</text>
        </g>
        <g ref={headRef} className="mp-head" data-x={STAR.x} data-y={STAR.y} style={{ display: "none" }}>
          <circle r="8" />
