...
export const Route = createFileRoute("/villas/$type")({
  head: ({ params }) => {
    const type = (params as { type: string }).type.toUpperCase() as TypeVilla;
    const t = textes[LANGUE];
    const chiffres = villasChiffres[type] ?? villasChiffres.A;
    const chemin = "/villas/" + (params as { type: string }).type.toLowerCase();
    const autre = "/en/villas/" + (params as { type: string }).type.toLowerCase();
    return {
      meta: [
